...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(shape=(None, self.config.window_size), dtype=tf.int32, name="X")
    self.labels_placeholder = tf.placeholder(shape=(None, self.config.label_size), dtype=tf.float32, name="Y")
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      L = tf.get_variable(name="embedding_matrix", shape=[len(self.wv), self.config.embed_size], dtype=tf.float32)
  ...
  def add_model(self, window):
    ...
    self.regularizer = tf.contrib.layers.l2_regularizer(scale = self.config.l2)
    
    with tf.variable_scope("Layer"):
      W = tf.get_variable(shape=[self.config.window_size*self.config.embed_size, self.config.hidden_size], name="W", dtype=tf.float32, initializer=initializer, regularizer = self.regularizer)
      b1 = tf.get_variable(shape=[self.config.hidden_size], name="b1", dtype=tf.float32, initializer = tf.zeros_initializer)

    with tf.variable_scope("Softmax"):
      U = tf.get_variable(shape=[self.config.hidden_size, self.config.label_size], name="U", dtype=tf.float32, initializer=initializer, regularizer = self.regularizer)
      b2 = tf.get_variable(shape=[self.config.label_size], name="b2", dtype=tf.float32, initializer=tf.zeros_initializer)


    hidden_layer_output = tf.nn.tanh(tf.matmul(window, W) + b1)
    hidden_layer_output_dropout = tf.nn.dropout(hidden_layer_output, keep_prob = self.dropout_placeholder)

    softmax_output = tf.matmul(hidden_layer_output_dropout, U) + b2
    #softmax_output_dropout = tf.nn.dropout(softmax_output, keep_prob=self.config.dropout)
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=y, labels=self.labels_placeholder)) + reg_term
