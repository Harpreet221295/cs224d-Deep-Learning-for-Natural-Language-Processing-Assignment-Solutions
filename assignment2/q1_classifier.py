...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(shape=(None,self.config.n_classes), dtype=tf.int64)
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope("softmax_simple_classifier"):
      W = tf.get_variable(name="W", shape = [self.config.n_features, self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
      b = tf.get_variable(name="b", shape=[self.config.n_classes], dtype=tf.float32, initializer=tf.zeros_initializer)
